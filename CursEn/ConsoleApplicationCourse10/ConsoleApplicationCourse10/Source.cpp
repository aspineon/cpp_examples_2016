...
public:
	...
	//method for serializing an object in a binary file
	void saveObject(ofstream& file) {
		if (file.is_open()) {
			//write the code
			file.write((char*) &this->code, sizeof(int));
			//write the name
			file.write((char*)this->name, sizeof(this->name));
			...
			////write the faculty name
			//use a static buffer scenario
			char buffer[100];
	...
	//reading an object from a binary file - deserializing
	void readObject(ifstream& file) {
		if (file.is_open()) {
			//reading the code
			file.read((char*) &this->code, sizeof(int));
			//reading the name
			file.read(this->name, sizeof(Student::name));
			//number of grades
			...
			delete [] this->faculty;
			this->faculty = new char[strlen(buffer) + 1];
			strcpy(this->faculty, buffer);
		}
	}
...
void main() {
	...
	//	s1.saveObject(saveFile);

	//	saveFile.close();

	//	cout << endl << " Object saved";
	//}
	ifstream readFile;
